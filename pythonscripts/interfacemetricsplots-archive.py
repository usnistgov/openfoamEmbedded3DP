...
def inkfunc(tp:Dict) -> float:
    return tp['nuink']

def supfunc(tp:Dict) -> float:
    return tp['nusup']

def ninkfunc(tp:Dict) -> float:
    return tp['nink']

def nsupfunc(tp:Dict) -> float:
    return tp['nsup']

def tau0inkfunc(tp:Dict) -> float:
    return tp['tau0ink']

def tau0supfunc(tp:Dict) -> float:
    return tp['tau0sup']

def kinkfunc(tp:Dict) -> float:
    return tp['kink']

def ksupfunc(tp:Dict) -> float:
    return tp['ksup']

def sigfunc(tp:Dict) -> float:
    return tp['sigma']
...
def vv(tp:Dict, xpv):
    ...
    if xpos<0 or ypos<0 or sigmapos<0:
        #print(x, xpv.xlist, y, xpv.ylist, sigma, xpv.sigmalist)
        raise ValueError
    if type(xpv) is comboPlot:
        x0 = xpv.xmlist[xpos]
        y0 = xpv.ymlist[ypos]
    else:
        x0 = xpos
...
#### WORKING WITH GRIDS
    ...
    # mode is 0 to plot viscosity vs. viscosity
        # 1 to plot v*v vs. v/v
        # 2 to plot sigma vs. v/v
        # 3 to plot support v vs. support v/ink v
        # 4 to plot support v vs. support v/ink v on 3 separate plots for 3 separate sigmas
        # 5 to plot viscosity vs. viscosity  on 3 separate plots for 3 separate sigmas



def getLabel(func, short):
    if func==inkfunc:
        label = 'Ink viscosity (Pa$\cdot$s)'
    elif func==supfunc:
        label = 'Support viscosity (Pa$\cdot$s)'
    elif func==tau0inkfunc:
        label = 'Ink yield stress (Pa)'
    elif func==tau0supfunc:
        label = 'Support yield stress (Pa)'
    elif func==kinkfunc:
        label = 'Ink k (Pa*s^n)'
    elif func==ksupfunc:
        label = 'Support k (Pa*s^n)'
    elif func==ninkfunc:
        label = 'Ink n'
    elif func==nsupfunc:
        label = 'Support n'
    elif func==multfunc:
        label = 'Support viscosity \u00d7 ink viscosity (Pa$^2\cdot$s$^2$)'
    elif func==divfunc:
        label = 'Support viscosity / ink viscosity'
    elif func==sigfunc:
        label = 'Surface tension (mJ/m$^2$)'
    if short:
...
class folderPlots:
    def __init__(self, topFolder, imsize, mode, **kwargs):
        self.kwargs = kwargs
    ...
    def plotsLists(self, **kwargs):
        ...
        self.flist, self.tplists = listTPvalues(self.flist, **kwargs) # list of ink viscosities, support viscosities, and surface tensions
        if self.mode==0:
            ...
            self.xfunc = tau0inkfunc
            self.yfunc = tau0supfunc
            self.cfunc = sigfuncc
        elif self.mode==7:
            self.xfunc = ninkfunc
            self.yfunc = nsupfunc
...
class gridOfPlots(folderPlots):
    def __init__(self, topFolder, imsize, mode, **kwargs):
        super(gridOfPlots, self).__init__(topFolder, imsize, mode, **kwargs)       
    ...
    def clean(self):
     ...
     #   self.fig.set_size_inches(self.imsize*len(self.xlistreal), self.imsize*len(self.ylistreal))
        ...
        self.fig.suptitle(getLabel(self.xfunc, False), y=0.92-(firsty/len(self.ylist)), fontsize=fs)
        self.fig.text((lastx/len(self.xlist))-0.1, 0.5, getLabel(self.yfunc, False),\
...
class comboPlot(folderPlots):
    def __init__(self, topFolder:str, xr:List[float], yr:List[float], imsize:float, mode:int, gridlines:bool=True, **kwargs):
        super().__init__(topFolder, imsize, mode, **kwargs)  
        ...
        # in mode 4, make a row of 3 plots. In modes 0-3, make one plot
        if mode==4 or mode==5:
        ...
        if not mode==5 and gridlines:
            for ax in axs:
                ax.grid(linestyle='-', linewidth='0.25', color='#949494')
        
        # diagonal gridlines
#         if mode==0:
#             for i in range(-len(self.xlist), len(self.xlist)+1):
#                 axs[0].plot([-100,100],[-100+i*self.dy,100+i*self.dy], color='#949494', linestyle='-', linewidth='0.25')
    ...
    def addLegend(self):
        if self.mode in [0,1,3,6,7,8]:
            self.axs[0].legend(handles=self.plist, loc='upper center', ncol=4, bbox_to_anchor=(0.5, self.titley))
    ...
    def clean(self):
        ...
        for ax in self.axs:
            ax.set_xlabel(getLabel(self.xfunc, len(self.axs)>1), fontname="Arial", fontsize=10)
        ...
        self.axs[0].set_ylabel(getLabel(self.yfunc, False), fontname="Arial", fontsize=10)
        
        if self.mode==2:
            yticklabels = self.ylist
        else:
        ...
        if self.ab and len(self.xlistreal)>0 and len(self.ylistreal)>0:
            self.fig.set_size_inches(self.imsize, self.imsize*len(self.ylistreal)/len(self.xlistreal)/len(self.axs))
       
        self.fig.suptitle(self.figtitle, y=self.titley-0.1, fontname="Arial", fontsize=10)
