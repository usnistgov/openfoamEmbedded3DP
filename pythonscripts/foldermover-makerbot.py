...
def doneFolder(topfolder:str, tfinal:float) -> None:
    ...
    for f in caseFolders(topfolder):
        ...
        if ct==0:  # if the simulation hasn't logged any steps yet
            if os.path.exists(os.path.join(f, 'log_interFoam')) or os.path.exists(os.path.join(f, 'case', 'log_interFoam')): # if the simulation has started
                if f0 in tdl: # if the simulation is still in the queue
                    progress.append(f0)
                else: # if the simulation was aborted at 0
                    aborted.append(f0)
            else: # if the simulation hasn't started
                notstarted.append(f0)
        elif ct>=tfinal: # if the simulation has finished at the final time
            done.append(f0)
        else: # if the simulation is partway through
            if f0 in tdl: # if the simulation is stil in the queue
                progress.append(f0)
            else: # if the simulation has been thrown out of the queue
                if ct>=1: # the simulation was aborted at 1
                    aborted1.append(f0)
                else: # the simulation was aborted before 1
                    aborted.append(f0)
    plog = []
    
    print('Populating legends')
    # go through all of the simulations that are running now or were previously started but not finished
    for p in progress:
        o = 0
        endtime = 2.5
        abort = '     '
        f = os.path.join(topfolder, p) # full path of the folder
        t = populate(f) # update legend
        t = importIf(f) # import legend
        runtime = t[5][1] # actual time in real life
        ct = t[6][1] # current time
        rate = t[7][1] # simulation rate
        if len(rate)>0: # only run this if we successfully calculated a simulation rate
            try:
                rate = float(rate)
                ct = float(ct) # ct = current time in simulation
                runtime = float(runtime) # actual time in real life (hr)
            except:
                # if there are any non-numeric values in those legend entries, give up
                pass
            else:
        ...
        print(os.path.basename(f), o, endtime, ct)
    ...
    aborted.sort()
    logging.info('###### done at 2.5: '+', '.join(done))
