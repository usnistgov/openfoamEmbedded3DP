...
def txtPlot(folder:str, cp:folderPlots, dt:float, fontsize:int=10) -> None:
...
def viscRatioPlot(folders:str, time:float, xbehind:float, xunits:str='mm', xvar:str='viscRatio', yvar:str='aspectratio', cvar:str='', mvar:str='', fontsize:int=8, **kwargs) -> None:
    ...
    cvar is the variable to color the points by. set cvar='' to only use one color
    mvar is the variable to set markers by. only works if cvar is also set
    '''
    ...
    for f in folders:
        try:
            ...
            logging.info(f'Failed to get metrics for {os.path.basename(f)}')
            pass
        else:
            vr = intm.viscRatio(f)   # get the viscosity ratio
            d = {**d, **vr}
            if not cvar=='' or not mvar=='':
                le, u = extractTP(f, units=True)
            if not cvar=='':
                d['cvar'] = float(le[cvar])  # get the color value
            else:
                d['cvar'] = 0
            if not mvar=='':
                d['mvar'] = float(le[mvar])  # get the marker value
            else:
                d['mvar'] = 0
    ...
    if not cvar=='':
        ...
        if not mvar=='':
            ...
            mlist = list(df['mvar'].unique())
            mlist.sort()
            markers = ['o','v','s','^','X','D','v']
            # all same marker
            for j,cval in enumerate(clist):
                for k,mval in enumerate(mlist):
                    dfi = df[(df['cvar']==cval)&(df['mvar']==mval)]
                    if len(dfi)>0:
                        ax.scatter(dfi[xvar], dfi[yvar], color=cm[j], marker=markers[k], label='{:.2f},{:.2f}'.format(cval, mval))
            legtitle = f'{varNickname(cvar, short=True, units=u)},{varNickname(mvar, short=True, units=u)}'
            ax.legend(title=legtitle, bbox_to_anchor=(1,1))
        else:
            # all same marker
            for j,cval in enumerate(clist):
                dfi = df[df['cvar']==cval]
                ax.scatter(dfi[xvar], dfi[yvar], color=cm[j], label=cval)
            ax.legend(title=varNickname(cvar, short=True, units=u), bbox_to_anchor=(1,1))
    else:
